...
class dop853 {
	public:
		/** The number of components in the differential equation
		 * system. */
		const int n;
		/** The maximum number of integration steps. */
		const long nmax;
		/** The number of timesteps between which to test for
		 * stiffness. */
		const long nstiff;
		/** A factor used in estimating the length of the next
		 * timestep. */
		const double fac1;
		/** A factor used in estimating the length of the next
		 * timestep. */
		const double fac2;
		/** A safety factor used in estimating the length of the next
		 * timestep. */
		const double safe;
		/** An exponent used in estimating the length of the next
		 * timestep. */
		const double beta;
		/** An estimate of the smallest number that can be added to 1
		 * to obtain a different number, used to determine when the
		 * timestep has become too small. */
		const double uround;
